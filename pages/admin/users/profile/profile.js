$(document)
	...
	.on('click','.change-pw',function() {
		$(this).fadeOut('fast');
		$('#pw-fields').slideDown('fast');
		$('input[type=password]').removeAttr('disabled');
	})

	.on('submit','form.form-profile',function(e) {
		e.preventDefault();
		...
		$.post('/admin/users/profile/ajax/save-profile', data, function(res) {
			if (res.success) {
				$('#message').html('Profile updated...').addClass('aql_saved');
			}
			else {
