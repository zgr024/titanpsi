...
function pieChart() {
  ...
  var pullOutLabelFont = "14px Khula";  // Pull-out slice label font
  var pullOutValueFont = "12px Khula";  // Pull-out slice value font
  ...
  var currentPullOutSlice = $('#finalStart1').val();     // The slice currently pulled out (-1 = no slice)
  var currentPullOutDistance = 10;   // How many pixels the pulled-out slice is currently pulled out in the animation
  ...
  function init() {
    ...
    $('.reveal-box').each( function() {
	  ...
	  chartData[currentRow]['label'] = '';
	  buttonID = $(this).text().toLowerCase();
	  $('#'+buttonID).data( 'slice', currentRow );
      $('#'+buttonID).on('click',( handleTableClick ));
      ...
      if ( rgb = $(this).data('color').match( /rgb\((\d+), (\d+), (\d+)/) ) {
        chartColours[currentRow] = [ rgb[1], rgb[2], rgb[3] ];
      } else if ( hex = $(this).data('color').match(/#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/) ) {
  ...
  function handleTableClick ( clickEvent ) {
	...
	$('.reveal-box.on').removeClass('on').addClass('off');
	$(this).removeClass('off').addClass('on');
	$('.results-box').text($(this).data('percent')+'%');
	$('#dynamic-arrow').fadeIn('fast').animate({top:arrowPosition[slice]});
	console.log(slice);
    toggleSlice ( slice );
  }
  ...
  function drawSlice ( context, slice ) {
    ...
    if ( slice == currentPullOutSlice ) {
      ...
      var actualPullOutDistance = currentPullOutDistance * easeOut( currentPullOutDistance/maxPullOutDistance, 1 );
